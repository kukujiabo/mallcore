  ...
  public function edit($data) {
    ...
    if ($mupdate || $uupdate) {
      ...
      if (IS_POSS) {
        ...
        $updated['pos_id'] = $acct['pos_id'];

        $pUpdate = array();

        foreach(MemberSv::POS_MAPPING_FIELDS as $pf => $cf) {

          if ($cf == 'sex') {

            $pUpdate[$pf] = $updated[$cf] == 1 ? '先生' : '女士';
          
          } else {
        
            $updated[$cf] ? $pUpdate[$pf] = $updated[$cf] : null;
         ...
         * 接口必须有手机号
         */
        $pUpdate['sMobile'] = $user['user_tel'];

        $syncResult = PosSv::updateMemberInfo($pUpdate);

        $resultArray = json_decode($syncResult, true);

        if (!$resultArray || $resultArray[0]['Status'] != 1) {
          ...
          throw new PossException(
            ...
            ErrorCode::PossSv['SYNC_POS_MEMBER_INFO_FAILED_MSG'] . $syncResult,

            ErrorCode::PossSv['SYNC_POS_MEMBER_INFO_FAILED_CODE'],
